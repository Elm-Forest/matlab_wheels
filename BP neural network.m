...
format short
%% 读取读取
%data=xlsread('BCHAIN-MKPRU3.xlsx','A36:AJ30');
data=xlsread('C:\Users\55378\Desktop\bp.xlsx');
%data=xlsread('数据.xlsx','Sheet1','A1:N252'); %%使用xlsread函数读取EXCEL中对应范围的数据即可  
%输入输出数据
input=data(:,6:end);   %data的第一列-倒数第二列为特征指标
output=data(:,1);  %data的最后面一列为输出的指标值
disp('全部样本数目:')
N=length(output)   %全部样本数目
disp('测试样本数目:')
testNum=900  %设定测试样本数目
disp('训练样本数目:')
...
for i=1:len
    ...
    prt(i,1)=i;
    prt(i,2)=output_test(i);
    prt(i,3)=test_simu(i);
    prt(i,4)=error(i);
end
%xlswrite('比特币BP预测结果.xlsx',prt)
